...
class ActivitiesDashboard {
    constructor() {
        ...
        if (!stateWasRestored) {
            ...
            setTimeout(() => {
                ...
                console.log(
                    "DEBUG: About to restore flexibility, pendingFlexibilityRestore =",
                    this.pendingFlexibilityRestore,
                );
                if (this.pendingFlexibilityRestore) {
                    console.log(
                        "Final flexibility restoration to:",
                        this.pendingFlexibilityRestore,
                    );
                    ...
                    setTimeout(() => {
                        ...
                        console.log(
                            "Final flexibility verification:",
                            finalCheck,
                        );
                        if (finalCheck !== this.pendingFlexibilityRestore) {
                            console.error(
                                "Flexibility restoration FAILED! Expected:",
                                this.pendingFlexibilityRestore,
                                "Got:",
                                finalCheck,
                            );
    ...
    async loadOffersCache() {
        try {
            ...
            if (data.success && data.data) {
                ...
                const uniqueVillas = [
                    ...new Set(
                        this.cachedOffers.map(
                            (offer) => offer.villa_display_name,
                        ),
                    ),
                ];
        ...
        try {
            ...
            const currentFlexibility =
                ...
                (() => {
                    ...
                    return selectedPill
                        ? selectedPill.dataset.flexibility
                        : "exact";
                })();
            ...
            console.log(
                "User state saved with flexibility:",
                currentFlexibility,
                "expanded villas:",
                expandedVillas,
                "selected offers:",
    ...
    loadUserState() {
        try {
            ...
            console.log(
                "DEBUG: Loaded flexibility value:",
                state.selectedFlexibility,
            );
            ...
            console.log(
                "DEBUG: Set pendingFlexibilityRestore to:",
                this.pendingFlexibilityRestore,
            );

            console.log("User state restored successfully");

            // Restore villa expansion and offer selection states after a longer delay
            // to ensure the DOM is fully rendered and all villa cards are loaded
            setTimeout(() => {
                console.log(
    ...
    getExpandedVillaStates() {
        ...
        const expandedButtons = document.querySelectorAll(
            ".show-details-btn.expanded",
        );

        console.log("DEBUG: Capturing expanded villa states");
        console.log("- Found expanded buttons:", expandedButtons.length);

        expandedButtons.forEach((button, index) => {
            ...
            if (card) {
                ...
                if (nameElement) {
                    ...
                    if (offer && offer.villa_display_name) {
                        console.log(
                            `- Villa name from offer: ${offer.villa_display_name}`,
                        );
        ...
        console.log("DEBUG: Captured expanded villas:", expandedVillas);
    ...
    getSelectedOfferStates() {
        ...
        activeButtons.forEach((button, index) => {
            ...
            if (card) {
                ...
                const villaButtons = card.querySelectorAll(
                    ".booking-option-btn",
                );
                ...
                if (villaName && buttonIndex >= 0) {
                    ...
                    console.log(
                        `- Villa: ${villaName}, Selected button index: ${buttonIndex}`,
                    );
                    console.log(`- Button classes: ${button.className}`);
                    console.log(
                        `- Total buttons in villa: ${villaButtons.length}`,
                    );
                }
            }
        });

        console.log("DEBUG: Captured selected offers:", selectedOffers);
    ...
    restoreVillaStates(state) {
        try {
            console.log("DEBUG: Starting villa state restoration");
            console.log("- State to restore:", state);
            console.log("- Expanded villas:", state.expandedVillas);
            console.log("- Selected offers:", state.selectedOffers);

            // Check what villa cards are currently available
            const allCards = document.querySelectorAll(".champion-offer-card");
            console.log("- Available villa cards:", allCards.length);
            allCards.forEach((card, index) => {
                const nameElement = card.querySelector(".villa-name-subtitle");
                const villaName = nameElement
            ...
            if (state.expandedVillas && state.expandedVillas.length > 0) {
                ...
                state.expandedVillas.forEach((villaName) => {
                    ...
                    allCards.forEach((card) => {
                        ...
                        if (
                            nameElement &&
                            nameElement.textContent.trim() === villaName
                        ) {
                            targetCard = card;
                        }
                    });
                    ...
                    console.log(
                        `- Found card for villa "${villaName}":`,
                        !!targetCard,
                    );

                    if (targetCard) {
                        const showDetailsBtn =
                            targetCard.querySelector(".show-details-btn");
                        console.log(
                            `- Found show details button:`,
                            !!showDetailsBtn,
                        );
                        console.log(
                            `- Button expanded state:`,
                            showDetailsBtn?.classList.contains("expanded"),
                        );
                        ...
                        if (
                            ...
                            console.log(
                                "✅ Restored expansion for villa:",
                                villaName,
                            );
                        } else {
                            console.log(
                                "⚠️ Villa already expanded or button not found:",
                                villaName,
                            );
                        }
                    } else {
                ...
                console.log("DEBUG: No villa expansions to restore");
            }

            // Restore selected offers within villas
            if (
                state.selectedOffers &&
                Object.keys(state.selectedOffers).length > 0
            ) {
                console.log("DEBUG: Restoring offer selections...");
                Object.entries(state.selectedOffers).forEach(
                    ([villaName, selectedButtonIndex]) => {
                        console.log(
                            `- Looking for villa "${villaName}" with selected button index: ${selectedButtonIndex}`,
                        );
                        ...
                        if (targetCard) {
                            ...
                            const buttons = targetCard.querySelectorAll(
                                ".booking-option-btn",
                            );

                            console.log(
                                `- Looking for button index: ${selectedButtonIndex}`,
                            );
                            console.log(
                                `- Available buttons in villa "${villaName}": ${buttons.length}`,
                            );
                            ...
                            if (selectedButtonIndex < buttons.length) {
                                ...
                                console.log(
                                    `- Found target button at index ${selectedButtonIndex}`,
                                );
                                console.log(
                                ...
                                if (
                                    ...
                                    console.log(
                                        `- Clicking button ${selectedButtonIndex} in villa "${villaName}"...`,
                                    );
                                    targetButton.click();
                                    console.log(
                                        "✅ Restored offer selection:",
                                        villaName,
                                        "-> button",
                                        selectedButtonIndex,
                                    );
                                    ...
                                    console.log(
                                        "⚠️ Offer already selected or button not found",
                                    );
                                }
                            } else {
                                console.log(
                                    `❌ Button index ${selectedButtonIndex} out of range (${buttons.length} buttons available)`,
                                );
                            }
                        } else {
                ...
                console.log("DEBUG: No offer selections to restore");
            }
        } catch (error) {
    ...
    getSelectedFlexibility() {
        ...
        if (allPills.length > 0) {
            allPills.forEach((pill, index) => {
                console.log(
                    `- Pill ${index}: data-flexibility="${pill.dataset.flexibility}", selected=${pill.classList.contains("selected")}`,
                );
            });
        }

        const flexibility = selectedPill
            ? selectedPill.dataset.flexibility
            : "exact";
    ...
    setSelectedFlexibility(flexibility) {
        ...
        pills.forEach((pill) => {
            ...
            if (pill.dataset.flexibility === flexibility) {
                ...
                console.log(
                    "Selected pill with flexibility:",
                    pill.dataset.flexibility,
                );
            }
        });
        ...
        console.log(
            ...
            selectedPill?.dataset.flexibility || "none",
        );
    }
    ...
    findExtensionOffers(
        ...
        let matchingOffers = this.cachedOffers.filter((offer) => {
            const offerCheckin = offer.checkin_date
                ? new Date(offer.checkin_date).toISOString().split("T")[0]
                : null;
        ...
        if (matchingOffers.length === 0) {
            console.log(
                "No exact match found, searching for compatible offers...",
            );
            ...
            matchingOffers = this.cachedOffers.filter((offer) => {
                const offerCheckin = offer.checkin_date
                    ...
                    : null;
                const matchesVilla =
                    offer.villa_display_name === villaDisplayName;
                const matchesDate = offerCheckin === checkinDate;
                const matchesAdults = offer.adults >= parseInt(adults);
                const matchesChildren = offer.children >= parseInt(children);
            ...
            if (matchingOffers.length > 0) {
                console.log(
                    `Found ${matchingOffers.length} compatible offers with different night counts`,
                );
                ...
                matchingOffers = matchingOffers.map((offer) => ({
                    ...
                    price_for_guests: parseFloat(
                        (
                            (parseFloat(offer.price_for_guests) /
                                offer.nights) *
                            parseInt(nights)
                        ).toFixed(2),
                    ),
                    total_face_value: parseFloat(
                        (
                            (parseFloat(offer.total_face_value) /
                                offer.nights) *
                            parseInt(nights)
                        ).toFixed(2),
                    ),
                    guest_savings_value: parseFloat(
                        (
                            (parseFloat(offer.guest_savings_value) /
                                offer.nights) *
                            parseInt(nights)
                        ).toFixed(2),
                    ),
    ...
    initializeEventListeners() {
        ...
        console.log(
            "DEBUG: initializeEventListeners() - Found",
            flexibilityPills.length,
            "flexibility pills",
        );
        flexibilityPills.forEach((pill, index) => {
            console.log(
                `DEBUG: Pill ${index}: data-flexibility="${pill.dataset.flexibility}", text="${pill.textContent}"`,
            );
            pill.addEventListener("click", (event) => {
                console.log(
                    "🔥 CLICK DETECTED ON PILL:",
                    pill.dataset.flexibility,
                    event,
                );
                ...
                const currentPills =
                    document.querySelectorAll(".flexibility-pill");
                console.log(
                    "DEBUG: Found",
                    currentPills.length,
                    "pills on click",
                ...
                console.log(
                    "DEBUG: Set selected class on pill with data-flexibility:",
                    pill.dataset.flexibility,
                );
    ...
    renderCalendar() {
        ...
        const existingDays =
            this.elements.calendarGrid.querySelectorAll(".calendar-date");
            ...
            if (isInRange) {
                ...
                if (currentDate.getTime() === baliTime.getTime()) {
                    ...
                    dayCell.addEventListener("click", () =>
                        this.handleDateClick(dateIndex),
                    );
                }
            } else {
    ...
    setDefaultDateSelection() {
        ...
        if (this.calendarDates.length >= 3) {
            ...
            console.log(
                "Default dates selected: Check-in tomorrow, Check-out day after tomorrow",
            );
    ...
    updateCalendarDisplay() {
        ...
        if (this.selectedCheckIn !== null) {
            const checkInCellIndex =
                this.calendarDates[this.selectedCheckIn]?.cellIndex;
            if (checkInCellIndex !== undefined) {
                const checkInElement = this.elements.calendarGrid.querySelector(
                    `[data-cell-index="${checkInCellIndex}"]`,
                );
        ...
        if (this.selectedCheckOut !== null) {
            const checkOutCellIndex =
                this.calendarDates[this.selectedCheckOut]?.cellIndex;
            if (checkOutCellIndex !== undefined) {
                const checkOutElement =
                    this.elements.calendarGrid.querySelector(
                        `[data-cell-index="${checkOutCellIndex}"]`,
                    );
                ...
                for (let i = 0; i < this.calendarDates.length; i++) {
                    if (i > this.selectedCheckIn && i < this.selectedCheckOut) {
                        ...
                        if (rangeCellIndex !== undefined) {
                            const rangeElement =
                                this.elements.calendarGrid.querySelector(
                                    `[data-cell-index="${rangeCellIndex}"]`,
                                );
                            if (rangeElement)
        ...
        console.log(
            "Calendar display updated - CheckIn:",
            this.selectedCheckIn,
            "CheckOut:",
            this.selectedCheckOut,
        );
    ...
    setDefaultDates() {
        ...
        if (tomorrowIndex !== -1 && dayAfterTomorrowIndex !== -1) {
            ...
            console.log(
                "No default dates available - calendar dates required for search",
            );
    ...
    changeGuestCount(type, delta) {
        const current =
            type === "adults" ? this.selectedAdults : this.selectedChildren;
        const minValue = type === "adults" ? 1 : 0;
        const maxValue = type === "adults" ? this.maxAdults : this.maxChildren;

        const newValue = Math.max(
            minValue,
            Math.min(maxValue, current + delta),
        );
    ...
    updateGuestButtons() {
        ...
        this.elements.adultsPlusBtn.disabled =
            this.selectedAdults >= this.maxAdults;

        // Update children buttons
        this.elements.childrenMinusBtn.disabled = this.selectedChildren <= 0;
        this.elements.childrenPlusBtn.disabled =
            this.selectedChildren >= this.maxChildren;
    }
    ...
    setupDynamicDateFilters() {
        ...
        let filters;
        if ([1, 2, 3, 6, 0].includes(dayOfWeek)) {
            // Mon, Tue, Wed, Sat, Sun
    ...
    updateFilterBoxes(filters) {
        Object.keys(filters).forEach((filterKey) => {
            ...
            if (element) {
                ...
                element.querySelector(".filter-title").textContent =
                    filter.title;
                element.querySelector(".filter-subtitle").textContent =
                    filter.subtitle;
                ...
                if (filter.dateRange === "custom") {
                    ...
                    element.dataset.dateRange = JSON.stringify([
                        startStr,
                        endStr,
                    ]);
                }
    ...
    setActiveFilter(filterId) {
        ...
        ["filter1", "filter2", "filter3", "filter4"].forEach((id) => {
            this.elements[`dateFilter${id.slice(-1)}`].classList.remove(
                "active",
            );
        });

        // Add active class to selected filter
        this.elements[`dateFilter${filterId.slice(-1)}`].classList.add(
            "active",
        );
    }
    ...
    handleGuestCountChange() {
        console.log(
            `Guest count changed: ${this.selectedAdults} adults, ${this.selectedChildren} children`,
        );
    ...
    updateDualSliderDisplay() {
        ...
        this.elements.selectedStartDate.textContent =
            startOffset === 1 ? "Tomorrow" : startDateStr;
        ...
        const endDateStr = endBaliTime.toLocaleDateString(
            "en-US",
            startOptions,
        );
    ...
    async loadActivities(silent = false) {
        try {
            ...
            console.log(
                `Received ${data.data.length} available offers:`,
                data.metadata,
            );
    ...
    async loadMoreOffers() {
        ...
        console.log(
            "Load more functionality disabled - all offers loaded at once",
        );
        return;
    }
    ...
    selectBookingOption(offerId, villaKey, optionIndex) {
        console.log(
            `Selecting booking option ${optionIndex} for villa ${villaKey}, offer ${offerId}`,
        );

        // Find the villa card
        const villaCard = document
            .querySelector(`[data-offer-id="${offerId}"]`)
            .closest(".champion-offer-card");
        if (!villaCard) {
        ...
        const bookingButtons = villaCard.querySelectorAll(
            ".booking-option-btn",
        );
        ...
        if (selectedButton) {
            ...
            if (bookButton) {
                ...
                const updatedOnclick = currentOnclick.replace(
                    /handleChampionBooking\('([^']+)'/,
                    `handleChampionBooking('${newOfferId}'`,
                );
    ...
    generateOffersFromData(data) {
        ...
        let availableData = workingData.filter(
            (record) => record.AvailabilityCount > 0,
        );
        ...
        console.log(
            `Generating offers from ${availableData.length} available records (${dateRangeDesc}) for ${this.selectedAdults} adults, ${this.selectedChildren} children`,
        );

        if (availableData.length === 0) {
            console.log(
                "No available rooms found matching your guest requirements and date range",
            );
            return [];
        }
        ...
        Object.keys(villaGroups).forEach((villaName) => {
            const records = villaGroups[villaName].sort(
                (a, b) =>
            ...
            records.forEach((checkInRecord, index) => {
                ...
                console.log(
                    `Creating ${maxNights} night options for ${villaName} starting ${checkInDate} (consecutive days: ${consecutiveDays})`,
                );
                ...
                for (let nights = 1; nights <= maxNights; nights++) {
                    ...
                    const checkOutDateStr = checkOutDateObj
                        .toISOString()
                        .split("T")[0];
                    ...
                    for (let dayOffset = 0; dayOffset < nights; dayOffset++) {
                        ...
                        const isDateAvailable = records.some(
                            (record) =>
                                record.EntryDate.split("T")[0] ===
                                    testDateStr && record.AvailabilityCount > 0,
                        );
                    ...
                    if (allDatesAvailable) {
                        offers.push({
                            ...
                            villa_display_name:
                                checkInRecord.villa_display_name,
                        });
                    } else {
                        console.log(
                            `${nights} nights not available for ${villaName} starting ${checkInDate} - missing availability`,
                        );
                    }
                }
            });
    ...
    async displayActivities(availableOffers) {
        ...
        console.log(
            "Stored currentOffers with offer_ids:",
    ...
    async generateChampionCards(offers) {
        ...
        const villaCardsHtml = await Promise.all(
            Object.entries(villaGroups).map(async ([villaKey, villaOffers]) => {
                ...
                console.log(
                    "All options:",
                    villaOffers.map(
                        (o) => `${o.checkIn.split("T")[0]} (${o.nights}n)`,
                    ),
                );

                // Generate single card with all booking options for this villa
                const cardHtml =
                    await this.generateChampionVillaCardWithTimeline(
                        villaKey,
                        primaryOffer,
                        sortedVillaOffers,
                    );
                console.log(
                    `Generated HTML for ${villaKey}, length: ${cardHtml ? cardHtml.length : 0} chars`,
                );
                if (cardHtml && cardHtml.length < 500) {
                    console.log(
                        `WARNING: Short HTML for ${villaKey}:`,
                        cardHtml,
                    );
                }
                return cardHtml;
        ...
        console.log(
            "HTML lengths:",
            villaCardsHtml.map((h) => (h ? h.length : 0)),
        );
        ...
        // Verify insertion
        const insertedCards = championOffersElement.querySelectorAll(
            ".villa-card, .champion-offer-card",
        );
    ...
    generateBookingOptionsForOffer(primaryOffer, allOffers, villaKey) {
        ...
        console.log(
            `Found ${uniqueOptions.length} unique booking options for villa ${villaKey}`,
        );
        ...
        if (uniqueOptions.length === 1) {
            ...
            const optionButtons = uniqueOptions
                .map((option, index) => {
     ...
     */
    async generateChampionVillaCardWithTimeline(
        villaKey,
        primaryOffer,
        allVillaOffers,
    ) {
        ...
        console.log("Primary offer data:", {
            ...
            image_urls: primaryOffer.image_urls
                ? primaryOffer.image_urls.substring(0, 100) + "..."
                : "NO IMAGE URLS",
            hasImageUrls: !!primaryOffer.image_urls,
        });

        const tagline =
            primaryOffer.tagline || primaryOffer.villa_display_name || villaKey;
        const imageUrls = this.parseImageUrls(primaryOffer.image_urls);
        console.log(
            `Parsed ${imageUrls.length} image URLs for ${villaKey}:`,
            imageUrls,
        );
        ...
        if (primaryOffer.savings && primaryOffer.savings > 0) {
            const savingsInMillions = (primaryOffer.savings / 1000000).toFixed(
                1,
            );
            const savingsPercent = Math.round(
                primaryOffer.savingsPercent * 100,
            );
            savingsBanner = `
        ...
        const monthName = checkInDate.toLocaleDateString("en-US", {
            month: "short",
        });
        ...
        if (primaryOffer.perks_included) {
            const perks = primaryOffer.perks_included
                .split(",")
                .map((p) => p.trim());
            const perkIcons = {
        ...
        const bookingOptionsHtml = this.generateBookingOptionsForOffer(
            primaryOffer,
            allVillaOffers,
            villaKey,
        );
    ...
    async extendStay(offerId, villaKey, extensionDate, extensionType) {
        ...
        if (confirm(confirmMessage)) {
            try {
                ...
                const newDateRange = this.calculateExtendedDateRange(
                    currentCard,
                    extensionDate,
                    extensionType,
                );
                ...
                if (extendedOffer) {
                    ...
                    await this.updateVillaCardWithExtendedOffer(
                        currentCard,
                        extendedOffer,
                        villaKey,
                        [extendedOffer],
                    ...
                    console.log(
                        "No matching offer found in cache for extension",
                    );
                    this.hideExtensionLoadingState(currentCard);
                    alert(
                        "Sorry, this villa is not available for the extended dates. Please try different dates.",
                    );
                }
            } catch (error) {
                console.error("Error extending stay:", error);
                alert(
                    "Sorry, there was an error extending your stay. Please try again.",
                );
            }
        }
    }
    ...
    updateCalendarSelection(newCheckinDate, newCheckoutDate) {
        ...
        for (let i = 0; i < this.calendarDates.length; i++) {
            const calDate = this.calendarDates[i].date
                .toISOString()
                .split("T")[0];
        ...
        console.log(
            "Updated calendar selection: check-in index",
            this.selectedCheckIn,
            "check-out index",
            this.selectedCheckOut,
        );
    ...
    calculateExtendedDateRange(currentCard, extensionDate, extensionType) {
        console.log(
            "Calculating extended date range for extension:",
            extensionDate,
            "type:",
            extensionType,
        );
        ...
        console.log(
            "Current booking from card: check-in",
            currentCheckinDate.toISOString().split("T")[0],
            "check-out",
            currentCheckoutDate.toISOString().split("T")[0],
        );
        ...
        if (extensionType === "before") {
            ...
            console.log(
                "Backward extension: moving check-in to",
                extensionDate,
            );
        } else {
            ...
            console.log(
                "Forward extension: extending check-out by 1 day from",
                currentCheckoutDate.toISOString().split("T")[0],
                "to",
     ...
     */
    async getExtendedOfferFromCache(
        villaDisplayName,
        checkinDate,
        nights,
        adults,
        ...
        const cachedOffers = this.findExtensionOffers(
            villaDisplayName,
            checkinDate,
            nights,
            adults,
        ...
        const processedOffer = {
            ...
            checkOut: this.calculateCheckoutDate(
                offer.checkin_date,
                offer.nights,
            ),
            ...
            perk_ids: offer.perk_ids
                ? typeof offer.perk_ids === "string"
                    ? JSON.parse(offer.perk_ids)
                    : offer.perk_ids
                : [],
    ...
    getCurrentOfferFromCard(cardElement) {
        ...
        const checkIn = this.calendarDates[this.selectedCheckIn].date
            .toISOString()
            .split("T")[0];
        const checkOut = this.calendarDates[this.selectedCheckOut].date
            .toISOString()
            .split("T")[0];
    ...
    showNextGalleryImage() {
        ...
        this.galleryModal.currentIndex =
            (this.galleryModal.currentIndex + 1) % currentImages.length;
        this.galleryModal.modalImg.src =
            currentImages[this.galleryModal.currentIndex];
    }
     ...
     */
    async updateVillaCardWithExtendedOffer(
        currentCard,
        newOffer,
        villaKey,
        allExtendedOffers,
        ...
        try {
            ...
            console.log(
                `Re-querying for ${villaKey} from ${queryStartDate.toISOString().split("T")[0]} to ${queryEndDate.toISOString().split("T")[0]}`,
            );
            ...
            console.log(
                "Ignoring re-query, using full cache to find all villa offers for extension checking",
            );

            // Get ALL cached offers for this villa across all dates
            const allCachedVillaOffers = this.cachedOffers.filter(
                (offer) => offer.villa_display_name === villaKey,
            );

            console.log(
                `Found ${allCachedVillaOffers.length} total cached offers for ${villaKey}`,
            );
            ...
            const newCard = document.querySelector(
                `[data-offer-id="${newOffer.offer_id}"]`,
            );
            if (newCard) {
            ...
            console.log(
                "Villa card updated with extended offer and all cached extension options",
            );
        } catch (error) {
            console.error("Error re-querying offers:", error);

            // Fallback to original logic
            const villaOffers = allExtendedOffers.filter(
                (offer) =>
                    offer.villa_display_name === villaKey ||
                    offer.villa === villaKey,
            );
            ...
            console.log(
                "Villa card updated with extended pricing and perks (error fallback)",
            );
        }
    }
    ...
    handleChampionBooking(offerId, villaKey) {
        ...
        console.log(
            "Available offer IDs in currentOffers:",
            this.currentOffers.map((o) => o.offer_id),
        );
        ...
        if (!offer) {
            ...
            console.error(
                "Available offers with IDs:",
                this.currentOffers.map((o) => ({
                    offer_id: o.offer_id,
            ...
            alert(
                "Sorry, this offer is no longer available. Please refresh the page and try again.",
            );
            return;
        }
        ...
        const bookingData = {
            ...
            totalGuests:
                (this.selectedAdults || 2) + (this.selectedChildren || 0),
            rate: offer.rate,
    ...
    selectChampionOffer(offerId, villaKey) {
        ...
        const card = document
            .querySelector(`[data-offer-id="${offer.offer_id}"]`)
            .closest(".champion-offer-card");
        if (!card) return;
    ...
    updateChampionCardDisplay(card, offer, villaKey) {
        ...
        const priceAmount = card.querySelector(
        ...
        if (bookBtn) {
            ...
            bookBtn.onclick = () =>
                this.handleChampionBooking(offer.offer_id, villaKey);
        }
        ...
        if (offer.perks_included !== this.lastDisplayedPerks) {
            const perksContainer = card.querySelector(
                ".champion-perks-section",
            );
            if (perksContainer && offer.perks_included) {
                const perks = offer.perks_included
                    .split(",")
                    .map((p) => p.trim());
                const perkIcons = {
                    Cocktails: "fas fa-cocktail",
    ...
    async generateVillaCard(villaName, villaDetails, villaOffers) {
        ...
        const description =
            villaDetails.description || this.getVillaDescription(villaName);
        ...
        const checkinGroups = await Promise.all(
            Object.keys(villaOffers)
                ...
                .map(async (checkInDate) => {
                    ...
                    const defaultOffer =
                        dateOffers.find((offer) => offer.nights === 2) ||
                        dateOffers[0];
                    const villaId = this.mapVillaNameToId(villaName);

                    // Get perks for default selection
                    const defaultPerks = await this.fetchPerks(
                        villaId,
                        defaultOffer.nights,
                        this.selectedAdults,
                        this.selectedChildren,
                    );
                    const defaultPerksDisplay =
                        this.formatPerksDisplay(defaultPerks);
                    const defaultFaceValue = this.calculateFaceValue(
                        defaultOffer.rate,
                        defaultOffer.nights,
                        defaultPerks,
                        this.selectedAdults,
                    ...
                    const availableNights = dateOffers
                        .map((offer) => offer.nights)
                        .sort((a, b) => a - b);
                    console.log(
                        `Available nights for ${villaName} on ${checkInDate}:`,
                        availableNights,
                    );

                    const nightSelector = dateOffers
                        .map((offer) => {
                            const isActive =
                                offer.nights === defaultOffer.nights;
    ...
    getVillaDescription(villaName) {
        const descriptions = {
            "Shore Villa":
                "An exclusive sanctuary for the discerning traveler. Our two-bedroom Shore Villa offers unparalleled privacy and luxury.",
            "Garden Villa":
                "Immerse yourself in tropical tranquility. Our Garden Villa provides an intimate escape surrounded by lush landscapes.",
            "Pool Villa":
                "Experience ultimate luxury with your private pool. Our Pool Villa offers the perfect blend of comfort and exclusivity.",
            "Ocean Villa":
                "Wake up to breathtaking ocean views. Our Ocean Villa delivers an unforgettable beachfront experience.",
            "Sunset Villa":
                "Witness spectacular sunsets from your private terrace. Our Sunset Villa creates magical evening moments.",
        };

        return (
            descriptions[villaName] ||
            "A luxury villa experience crafted for your perfect getaway on Gili Air."
        );
    }
    ...
    populateCustomTableData(activities) {
        ...
        activities.forEach((activity) => {
            ...
            if (poolValue.includes("private")) {
                poolCell.innerHTML =
                    '<i class="fas fa-lock" style="color: #AA7831; margin-right: 5px;"></i>Private';
            } else if (poolValue.includes("shared")) {
                poolCell.innerHTML =
                    '<i class="fas fa-users" style="color: #4CAF50; margin-right: 5px;"></i>Shared';
            } else {
            ...
            const classValue = (activity.UserDefinedClass || "Normal")
                .toString()
                .toLowerCase();
            const classDisplay = activity.UserDefinedClass || "Normal";
            const classCSS = classValue.includes("premium")
                ? "premium"
                : "normal";
    ...
    parseImageUrls(imageUrlsField) {
        console.log(
            ...
            imageUrlsField
                ? imageUrlsField.substring(0, 100) + "..."
                : "NULL/EMPTY",
        );
        ...
        try {
            ...
            if (imageUrlsField.startsWith("[")) {
                ...
                console.log(
                    `parseImageUrls: Successfully parsed ${parsed.length} URLs from JSON`,
                );
                return parsed;
            }
    ...
    formatVillaSpecs(villaDetails) {
        ...
        if (villaDetails.square_meters) {
            specs.push(
                `<div class="spec-item"><i class="fas fa-expand-arrows-alt"></i> ${villaDetails.square_meters}m²</div>`,
            );
        }

        if (villaDetails.bathrooms) {
            specs.push(
                `<div class="spec-item"><i class="fas fa-bath"></i> ${villaDetails.bathrooms} ${villaDetails.bathrooms === 1 ? "Bath" : "Baths"}</div>`,
            );
        }

        if (villaDetails.view_type) {
            specs.push(
                `<div class="spec-item"><i class="fas fa-eye"></i> ${villaDetails.view_type}</div>`,
            );
        }

        if (villaDetails.pool_type) {
            specs.push(
                `<div class="spec-item"><i class="fas fa-swimming-pool"></i> ${villaDetails.pool_type} Pool</div>`,
            );
        }
    ...
    toggleDescription(button) {
        ...
        const isExpanded =
            descriptionContainer.getAttribute("data-expanded") === "true";
        ...
        const updateGuestSelectLabels = () => {
            ...
            if (adultsSelect) {
                ...
                adultsOptions.forEach((option) => {
                    if (isMobile) {
                        ...
                        option.textContent =
                            option.getAttribute("data-desktop");
                    }
                });
            }
            ...
            if (childrenSelect) {
                const childrenOptions =
                    childrenSelect.querySelectorAll("option");
    ...
    async fetchPerks(villaId, nights, adults, children) {
        try {
            const response = await fetch(
                `/api/perks/${villaId}/${nights}/${adults}/${children}`,
            );
            if (response.ok) {
                const data = await response.json();
    ...
    isPerPersonActivity(comments) {
        ...
        if (
            lowerComments.includes("per pax") ||
        ...
        if (
            lowerComments.match(/^\d+\s*pax$/i) || // "2 pax", "3 pax"
            lowerComments.match(/^\d+-\d+\s*pax$/i)
        ) {
            // "2-3 pax"
            return false;
        }
    ...
    async selectNights(cardId, nights, villaName, checkInDate) {
        try {
            ...
            if (!offer) {
                ...
                console.log(
                    "All offers for villa:",
                    villaOffers.filter((o) => o.villa === villaName),
                );

                // Show user-friendly error
                this.showNotification(
                    `Sorry, ${nights} night${nights === 1 ? "" : "s"} not available for this date`,
                    "error",
                );
                return false;
            }
            ...
            const villaId = this.mapVillaNameToId(villaName);
            const perks = await this.fetchPerks(
                villaId,
                nights,
                this.selectedAdults,
                this.selectedChildren,
            ...
            const faceValue = this.calculateFaceValue(
                offer.rate,
                nights,
                perks,
                this.selectedAdults,
            ...
            const faceValueElement = card.querySelector(
                '[data-field="face-value"]',
            );
            ...
            if (bookBtn) {
                bookBtn.onclick = () =>
                    this.handleBooking(villaName, checkInDate, nights);
            }
        } catch (error) {
            console.error("Error updating night selection:", error);
            this.showNotification(
                "Error updating selection. Please try again.",
                "error",
            );
        }
        return false;
    ...
    handleBooking(villaName, checkInDate, nights) {
        console.log(
            `Booking: ${villaName}, Check-in: ${checkInDate}, Nights: ${nights}`,
        );
        this.showNotification(
            `Booking ${villaName} for ${nights} night${nights === 1 ? "" : "s"} starting ${this.formatDateForDisplay(checkInDate)}`,
            "success",
        );
    }
    ...
    toggleVillaDetails(button) {
        const card = button.closest(".champion-offer-card");
        const collapsibleSection = card.querySelector(
        ...
        if (isExpanded) {
            ...
            button.innerHTML =
                '<i class="fas fa-chevron-down"></i> Show Detailed Offer';
    ...
    selectChampionNights(cardId, offerId) {
        try {
            ...
            const selectedOffer = this.currentOffers.find(
            ...
            if (!selectedOffer) {
                ...
                this.showNotification(
                    "Error: Offer not found. Please refresh and try again.",
                    "error",
                );
                return false;
            }
            ...
            const showFaceValue =
                selectedOffer.faceValue > selectedOffer.totalRate;
            if (faceValueElement) {
                if (showFaceValue) {
            ...
            console.log(
                `Updated champion offer selection: ${selectedOffer.nights} nights, Offer ID ${offerId}`,
            );
        } catch (error) {
     ...
     */
    goToNextSlide() {
        this.heroCarousel.currentSlide =
            (this.heroCarousel.currentSlide + 1) %
            this.heroCarousel.totalSlides;
    ...
    updateCarouselDisplay() {
        ...
        slides.forEach((slide, index) => {
            slide.classList.toggle(
                "active",
                index === this.heroCarousel.currentSlide,
            );
        });
...
class MailingListHandler {
    ...
    async handleSubmit() {
        ...
        try {
            ...
            this.showMessage(
                "Failed to process signup. Please check your connection and try again.",
                "error",
            );
        } finally {
    ...
    showMessage(message, type) {
        if (!this.messageContainer) {
            console.error(
                "Message container not found - cannot show message:",
                message,
            );
            return;
        }
        ...
        const messageText =
            this.messageContainer.querySelector(".message-text");
        if (messageText) {
        ...
        if (type === "success" && this.form) {
            ...
            if (
                !this.messageContainer.querySelector(".escape-list-update-btn")
            ) {
    ...
    hideMessage() {
        if (this.messageContainer) {
            ...
            const updateButton = this.messageContainer.querySelector(
